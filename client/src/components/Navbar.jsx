...
// ═══════════════════════════════════════════════════════════════════════════
// v16 refinement: scroll compression, Escape-to-close, safe-area spacing, and stronger keyboard semantics.
// Navbar — MockMate v12
// Design: refined glass capsule on the existing ice-blue system, a real
// command-palette launcher (⌘K), a rebuilt "vault" profile dropdown, and a
// tighter wordmark. Same palette family as Home.jsx — only the craft changed.
//
// v11 change: the "New Interview" CTA moved from a heavy dark midnight-blue
// gloss button to a light glass button (white fill, crisp blue border, solid
// blue icon chip) so it reads as part of the same airy ice-blue family as
// the sign-in button and stat chips, instead of importing a foreign dark
// material. Hover states across every icon button (avatar, bell, goal ring,
// stats chip, utility buttons) are unified onto one tactile recipe: a
// consistent lift + blue-tinted glow + brightened fill, plus a real
// active/press state that was previously missing on most of them.
// ═══════════════════════════════════════════════════════════════════════════
...
const C = {
  ...
  // premium accent — used sparingly, only inside the dropdown "vault" header
  // and the command palette, to mark those two surfaces as elevated.
  violet400: '#8B7CFF',
  violet500: '#6C5CE8',
  violet700: '#4A3CC4',
...
// ─── Logomark — faceted gem/dialogue mark, fill responds to live IRS score ──
  ...
  return (
    <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
      <svg width={size} height={size} viewBox="0 0 40 40">
        ...
        <rect
          x="1.2" y="1.2" width="37.6" height="37.6" rx="6.5"
        ...
        <ellipse cx="24.2" cy="27.2" rx="3.7" ry="5.8" fill="#0059F9" opacity="0.4" filter={`url(#${shadowId})`} />
        ...
        <path
          ...
          fill={`url(#${pinId})`}
        />
        <ellipse cx="22.6" cy="17.05" rx="2" ry="1.4" fill="#FFFFFF" opacity="0.5" transform="rotate(-25 22.6 17.05)" />
      </svg>
    </div>
  );
};
...
const Navbar = () => {
  ...
  // IRS delta this week — falls back to null (hidden) until the backend sends it.
  const irsDelta = user?.irsDeltaWeek ?? null;

  // Daily goal — backend can send { target, completed }. Defaults to a
  // 3-interview daily target so the ring works even if the field is missing.
  ...
  // Notifications — backend can send user.notifications: [{id,type,text,time,read}].
  // Falls back to streak-at-risk / feedback-ready heuristics derived from
  // fields that already exist, so the bell is meaningful with zero new API work.
    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        ...
        @keyframes mmCmdkIn { from { opacity:0; transform:translateY(-10px) scale(.98); } to { opacity:1; transform:translateY(0) scale(1); } }
        ...
        /* ── Main capsule — refined white glass ── */
        .mm-capsule {
          position: relative;
        ...
        /* Smoothly tighten the chrome while scrolling without moving the content. */
        ...
        /* ── Brand — unified logomark + wordmark lockup ── */
        .mm-brand {
        ...
        /* housing around the logomark ring — ties ring + text into one object */
        .mm-brand-ring {
          border-radius: 12px;
...
.mm-brand-tag {
  ...
  width: fit-content;
        ...
        /* Active pill — refined glass gloss */
        .mm-pill {
          position: absolute; top: 50%; height: 38px;
          border-radius: 10px;
          transform: translateY(-50%);
          background: linear-gradient(160deg,
            rgba(110,168,255,.22) 0%,
            rgba(26,110,255,.13) 45%,
            rgba(0,68,196,.10) 100%
          );
        ...
        /* CTA and avatar are heavier visual weight — give them a touch more room */
        .mm-right .mm-cta { margin-left: 3px; }
        .mm-right .mm-profile-wrap { margin-left: 2px; }

        /* ══════════════════════════════════════════════════════════════
           Primary interview CTA — logo-aligned action
           The logo's cyan → electric-blue gradient is now the primary
           action language, giving "New Interview" clear hierarchy.
           ══════════════════════════════════════════════════════════════ */
        .mm-cta {
          position: relative; overflow: hidden;
          display: inline-flex; align-items: center; justify-content: center; gap: 9px;
          height: 42px; padding: 0 16px 0 6px;
          border: 1px solid rgba(0,89,249,.78);
          border-radius: 13px;
          color: #fff;
          background: linear-gradient(135deg, #20C6F4 0%, #1A6EFF 46%, #0059F9 100%);
          font: 700 13px ${F.body};
          letter-spacing: -.012em; white-space: nowrap;
          cursor: pointer;
          box-shadow:
            inset 0 1px 0 rgba(255,255,255,.32),
            0 4px 10px rgba(0,89,249,.18),
            0 8px 22px rgba(26,110,255,.18);
        ...
        .mm-cta:hover {
          ...
          box-shadow:
            ...
            0 0 0 3px rgba(26,110,255,.10),
            0 10px 28px rgba(26,110,255,.28);
        }
        ...
        .mm-cta-icon {
          ...
          box-shadow: inset 0 1px 0 rgba(255,255,255,.24), 0 2px 6px rgba(0,31,107,.16);
          flex-shrink: 0;
        ...
        /* ── Profile button — blue halo + light inner avatar ── */
        .mm-profile-wrap { position: relative; flex-shrink: 0; }

        .mm-av-btn {
          display: inline-flex; align-items: center; justify-content: center;
          width: 41px; height: 41px;
          border: 1.5px solid rgba(26,110,255,.46);
          border-radius: 12px;
        ...
        .mm-av-btn:hover {
          border-color: rgba(0,89,249,.72);
          filter: brightness(1.03) saturate(1.05);
          box-shadow:
            0 0 0 3px rgba(26,110,255,.09),
            0 7px 18px rgba(26,110,255,.22);
          transform: translateY(-1.5px);
        }
        ...
        .mm-av-btn[aria-expanded="true"] {
          ...
          box-shadow:
            0 0 0 3px rgba(26,110,255,.14),
            0 7px 18px rgba(26,110,255,.18);
        }
        ...
        .mm-av-initial {
          ...
          color: ${C.blue700};
          font: 800 14px ${F.display};
          letter-spacing: -.01em;
          box-shadow: inset 0 1px 0 rgba(255,255,255,.92);
        }

        /* ══════════════════════════════════════════════════════════════
           Profile dropdown — "vault" redesign
           ══════════════════════════════════════════════════════════════ */
        .mm-drop {
          position: absolute;
        ...
        /* mesh header — the one place the violet accent is allowed */
        .mm-drop-mesh {
          position: relative;
        ...
        .mm-drop-tier {
          ...
          color: ${C.violet700};
          font: 700 9.5px ${F.body}; letter-spacing: .1px;
        }
        .mm-drop-tier-icon {
        ...
        .mm-drop-stats { display: grid; grid-template-columns: repeat(3,1fr); gap: 6px; margin-top: 12px; }
        .mm-drop-stat {
        ...
        .mm-drop-stat-label {
          font: 600 8px ${F.body}; letter-spacing: .2px; color: ${C.muted}; margin-top: 3px;
        ...
        .mm-drop-item-arrow { margin-left: auto; opacity: 0; transform: translateX(-3px); transition: all .16s ease; color: ${C.faint}; }
        ...
        /* ══════════════════════════════════════════════════════════════
           Utility icon buttons — bell + goal ring (only render when active)
           Unified hover recipe with the rest of the capsule: lift, brighten
           to full white, blue-tinted glow scaled down for the smaller
           footprint, plus a press state.
           ══════════════════════════════════════════════════════════════ */
        ...
        /* daily goal ring button — svg progress ring drawn directly on the icon */
        .mm-goal-btn { padding: 0; }
        .mm-goal-svg { transform: rotate(-90deg); }

        /* ── Popover shell (shared by bell / goal / stats) ── */
        .mm-popover {
        ...
        /* pointer nub connecting the popover to its trigger */
        .mm-popover::before {
          content: '';
        ...
        .mm-popover-inner { position: relative; border-radius: 18px; overflow: hidden; background: inherit; }
        .mm-popover-head {
        ...
        .mm-notif-text { font: 500 12.5px ${F.body}; color: ${C.sub}; line-height: 1.4; }
        .mm-notif-time { font: 500 10px ${F.body}; color: ${C.faint}; margin-top: 3px; }
        .mm-notif-empty {
          padding: 30px 16px; text-align: center;
          color: ${C.muted}; font: 500 12.5px ${F.body};
        }
        ...
        /* Stats chip (IRS + streak, always visible) */
        .mm-statschip {
        ...
        .mm-schip-label { font: 600 9px ${F.mono}; letter-spacing: .5px; text-transform: uppercase; color: ${C.muted}; line-height: 1; }
        ...
        .mm-stats-delta {
          ...
          margin-top: 5px; font: 650 11px ${F.body};
        }
        .mm-stats-grid { display: grid; grid-template-columns: repeat(2,1fr); gap: 8px; margin-top: 12px; }
        .mm-stats-cell { padding: 10px; border: 1px solid ${C.border}; border-radius: 13px; background: rgba(248,250,255,.7); transition: border-color .15s ease, transform .15s ease; }
        ...
        .mm-ham {
          ...
          cursor: pointer; transition: border-color .18s ease, background .18s ease, transform .18s cubic-bezier(.22,1,.36,1), box-shadow .18s ease;
        }
        ...
        .mm-mobile-panel {
          ...
          padding: 8px; border: 1px solid ${C.border}; border-radius: 20px;
        ...
        .mm-mobile-user {
          ...
          padding: 12px; border: 1px solid ${C.border}; border-radius: 15px;
        ...
        .mm-mobile-link.active {
          ...
          box-shadow: 0 6px 18px rgba(26,110,255,.3);
        }
        .mm-mobile-link-icon {
          width: 32px; height: 32px; display: flex; align-items: center; justify-content: center;
          border-radius: 10px; color: ${C.blue600}; background: ${C.blue50}; flex-shrink: 0;
        }
        ...
        /* Prevent tap/click highlight from fighting the capsule's tactile states. */
        .mm-focus { -webkit-tap-highlight-color: transparent; }

        .mm-brand:focus-visible { border-radius: 15px; }

        /* Keep the logo/action hierarchy crisp on very wide displays. */
      ...
      <nav className="mm-root" aria-label="Main navigation">
        <div className={`mm-capsule${scrolled ? ' scrolled' : ''}`}>
              ...
              <div ref={shellRef} className="mm-nav-track" role="list">
                ...
                {NAV_LINKS.map((link) => (
                  <Link
                    ...
                    to={link.path}
                    className={`mm-link mm-focus ${isActive(link.path) ? 'active' : ''}`} role="listitem"
          ...
          <div className="mm-right">
            {user ? (
              <>
                {/* Notification bell — only rendered when there's something unread */}
                {unreadCount > 0 && (
                ...
                {/* Daily goal ring — only rendered while today's goal is incomplete */}
                {!goalDone && (
                      ...
                      <div className="mm-popover" role="menu">
                        <div className="mm-popover-inner">
                          ...
                          <div className="mm-goal-body">
                            <svg width={54} height={54} viewBox="0 0 54 54" className="mm-goal-ring-big" style={{ transform: 'rotate(-90deg)' }}>
                          ...
                          <button type="button" className="mm-goal-cta mm-focus" onClick={() => { setGoalOpen(false); navigate('/interview'); }}>
                ...
                {/* IRS + streak chip — always visible, opens mini progress popover */}
                      ...
                      <div className="mm-popover-inner">
                        ...
                        <div className="mm-stats-body">
                          <div className="mm-stats-ring-row">
                            ...
                            <div>
                              <div className="mm-goal-copy-title" style={{ fontSize: 20, color: accent }}>{irs}<span style={{ fontSize: 12, color: C.muted, fontWeight: 600 }}> IRS</span></div>
                              {irsDelta !== null && (
                ...
                {/* Interview CTA */}
                <button type="button" className="mm-cta desktop-only mm-focus" onClick={() => navigate('/interview')}>
                    ...
                    <div className="mm-drop" role="menu">
                      ...
                      <div className="mm-drop-menu">
                        {[
                          ...
                          <button type="button" key={item.path} className="mm-drop-item mm-focus" role="menuitem"
                            style={{ animationDelay: `${i * 28}ms` }}
                            onClick={() => { setDropOpen(false); navigate(item.path); }}>
                            <span className="mm-drop-icon"><NavIcon name={item.icon} size={14}/></span>
                            {item.label}
                            <span className="mm-drop-item-arrow"><NavIcon name="chevDown" size={12}/></span>
                          </button>
                        ))}

                        <div className="mm-drop-divider"/>

                        <button type="button" className="mm-drop-item danger mm-focus" role="menuitem"
                ...
                <div className="mm-mobile-controls">
                  <button type="button" className="mm-cta mm-focus" style={{ height: 40, padding: '0 12px 0 5px' }}
                    onClick={() => navigate('/interview')}>
                    <span className="mm-cta-icon" style={{ width: 26, height: 26 }}><NavIcon name="mic" size={12}/></span>
          ...
          {mobileOpen && user && (
            <div className="mm-mobile-panel">
              {unreadCount > 0 && (
                <div className="mm-mobile-user" style={{ marginBottom: 8, background: '#FFF9F0', borderColor: 'rgba(234,88,12,.22)' }}>
                  <div className="mm-drop-avatar" style={{ width: 34, height: 34, borderRadius: 9, background: 'linear-gradient(145deg,#FFB74D,#EA580C)' }}>
                    <NavIcon name="bell" size={15}/>
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ font: `700 12px ${F.body}`, color: C.text }}>{notifications[0]?.text}</div>
                    {notifications.length > 1 && (
                      <div style={{ font: `500 10px ${F.body}`, color: C.muted, marginTop: 2 }}>+{notifications.length - 1} more</div>
                    )}
                  </div>
                </div>
              )}
              ...
              {!goalDone && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', marginBottom: 10, border: `1px solid ${C.border}`, borderRadius: 13, background: 'rgba(248,250,255,.7)' }}>
                  <svg width={34} height={34} viewBox="0 0 34 34" style={{ transform: 'rotate(-90deg)', flexShrink: 0 }}>
                    ...
                    <circle cx="17" cy="17" r="14" fill="none" stroke={C.blue500} strokeWidth="3.5"
                      strokeDasharray={2 * Math.PI * 14} strokeDashoffset={2 * Math.PI * 14 * (1 - goalPct / 100)} strokeLinecap="round"/>
                  </svg>
                  <div style={{ flex: 1 }}>
                    <div style={{ font: `700 12px ${F.body}`, color: C.text }}>Today's goal</div>
                    <div style={{ font: `500 10.5px ${F.body}`, color: C.muted, marginTop: 1 }}>{goalCompleted} of {goalTarget} interviews done</div>
                  </div>
                </div>
              )}
              ...
              <button type="button" className="mm-cta mm-focus" style={{ width: '100%', height: 42, marginTop: 8, justifyContent: 'center' }}
                onClick={() => { setMobileOpen(false); navigate('/interview'); }}>
                <span className="mm-cta-icon"><NavIcon name="mic" size={13}/></span>
                <span className="mm-cta-text">New Interview</span>
              </button>

              <button type="button" className="mm-mobile-link mm-mobile-logout mm-focus"
