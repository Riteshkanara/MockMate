...
const normalizeScore = session => {
  ...
  const direct = Number(session?.score);
  if (Number.isFinite(direct) && direct >= 0 && direct <= 100) return Math.round(direct);

  // NOTE: only use totalScore if it safely represents 0–100, derived from question count if needed.
  const total = Number(session?.totalScore);
  if (Number.isFinite(total) && total >= 0) {
    const qCount = Array.isArray(session?.questions)
      ? session.questions.length
      : Number(session?.questionCount || 0);
    if (total > 100 && qCount > 0) return Math.max(0, Math.min(100, Math.round(total / qCount)));
...
const scoreBg = score =>
  score >= 80 ? C.greenTint
  : score >= 60 ? C.blue50
  : score >= 40 ? C.amberTint
  : C.orangeTint;

const perfLabel = score =>
  score >= 80 ? 'Excellent' : score >= 60 ? 'Good' : 'Keep Practicing';

// — Helpers (extracted from render to keep effects clean — Pass 5A)

const buildStats = sessions => {
  const scores = sessions.map(normalizeScore).filter(Number.isFinite);
  ...
  return { total: sessions.length, average, best, trend, sparkValues: chrono.slice(-8) };
};

const buildTopicSparks = sessions => {
  const map = {};
  [...sessions]
    .sort((a, b) => new Date(a.createdAt || 0) - new Date(b.createdAt || 0))
    .forEach(s => {
      const key = (s.topic || s.company || 'General').toLowerCase().trim();
      if (!map[key]) map[key] = [];
      map[key].push(normalizeScore(s));
    });
  return map;
};
    ...
    <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
      <svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
        ...
        <circle
          ...
          strokeDashoffset={circ - (safe / 100) * circ}
          strokeLinecap="round"
          style={{ transition: 'stroke-dashoffset 1s cubic-bezier(.16,1,.3,1)' }}
        />
      </svg>
      <div style={S.ringLabel}>
        <span style={{ fontFamily: F.display, fontSize: 15, fontWeight: 800, color, lineHeight: 1 }}>
          {safe}
        </span>
      </div>
    </div>
...
const History = () => {
  ...
  const filtered = useMemo(() => {
    let result = filter === 'all'
      ? [...history]
      : history.filter(s => s.mode?.toLowerCase() === filter);
    result.sort((a, b) =>
      sort === 'score'
      ...
      <div style={S.container} className="history-container h-page">
        ...
        <section style={S.statsGrid} className="history-stats">
          ...
          <div style={S.statCard}>
            ...
            <div>
              ...
              <div style={{ ...S.statNumber, color: C.amber }}>
                {stats.best}<span style={S.scoreSuffix}>/100</span>
              </div>
            </div>
          </div>
        </section>
            ...
            <div style={S.filterGroup} className="history-filter">
              {FILTER_TABS.map(tab => (
                <button
                  ...
                  className={`h-filter-btn${filter === tab.value ? ' h-filter-active' : ''}`}
                  style={{ ...S.filterButton, ...(filter === tab.value ? S.filterButtonActive : {}) }}
                >
                  {tab.label}
                </button>
              ))}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {filtered.map((session, index) => {
            const score    = normalizeScore(session);
            const sColor   = scoreColor(score);
            const topicKey = (session.topic || session.company || 'General').toLowerCase().trim();
            const spark    = topicSparks[topicKey] || [];
            return (
              <article
                ...
                style={{
                  ...S.historyCard,
                  animation: mounted
                    ? `hFadeUp 0.4s ease ${Math.min(index * 0.04, 0.4)}s backwards`
                    : 'none',
                }}
              >
                <div style={S.historyMain}>
                  ...
                  <div style={S.historyInfo}>
                    ...
                    <h3 style={S.historyTitle}>
                      {session.topic || session.company || 'General Interview'}
                    </h3>
                    <div style={S.metaRow}>
                      <span>📅 {formatDate(session.createdAt)}</span>
                      <span>🕐 {formatTime(session.createdAt)}</span>
                      <span style={{ fontFamily: F.mono, color: C.blue500, fontWeight: 700 }}>
                ...
                <div style={S.scoreSection} className="history-score">
                  ...
                  <div style={{ ...S.performanceLabel, color: sColor }}>{perfLabel(score)}</div>
                </div>
              </article>
            );
          })}
...
const GLOBAL_CSS = C => `
  * { box-sizing: border-box; }
  ...
  .h-page select:focus-visible { outline: 2px solid ${C.blue500}; outline-offset: 2px; }
...
const S = {
  ...
  filterButtonActive: {
    background: C.card,
    color: C.blue600,
    boxShadow: '0 1px 4px rgba(0,87,232,0.14)',
    fontWeight: 700,
  },
  sortSelect: {
    border: `1px solid ${C.border}`,
  ...
  historyCard: {
    ...
    border: `1px solid ${C.border}`,
    borderRadius: 18,
    boxShadow: '0 1px 12px rgba(26,110,255,0.06)',
    cursor: 'default',
  },
  historyMain: {
    display: 'flex',
  ...
  emptyButton: {
    ...
    fontWeight: 700,
    cursor: 'pointer',
    boxShadow: '0 6px 18px rgba(0,87,232,0.26)',
    fontFamily: F.body,
  },
  errorBox: {
    display: 'flex',
